...
import { describe, expect, it } from 'vitest'
import {
  LANGUAGE_COLORS, FALLBACK_LANGUAGE_COLOR,
...
import { capsuleBg, capsuleText, coverBase, languageColor } from '../theme/languageColors'
...
function splitName(name: string): [string, string?] {
  ...
  return [name.slice(0, 12) + '-', name.slice(12).slice(0, 14)]
}
...
export default function RepoCover({ name, language, topics, coverUrl, className }: Props) {
  ...
  return (
    <svg className={`repo-cover ${className ?? ''}`} viewBox="0 0 672 378" role="img" aria-label={name}>
      ...
      <text x="48" y="160" fontSize="40" fontWeight="500" fill="#2F2A22">{line1}</text>
      {line2 && <text x="48" y="212" fontSize="40" fontWeight="500" fill="#2F2A22">{line2}</text>}
      {shown.map((t, i) => (
        <g key={t}>
...
describe('RepoCard', () => {
  it('渲染标题、tagline、作者与链接', () => {
    render(<RepoCard data={card} />)
    expect(screen.getByText('mini-agent')).toBeInTheDocument()
    expect(screen.getByText('给 LLM Agent 的最小运行时')).toBeInTheDocument()
    expect(screen.getByText('alice')).toBeInTheDocument()
    expect(screen.getByRole('link')).toHaveAttribute('href', '/repo/1')
  })
  it('常驻 star 与悬停统计', () => {
    render(<RepoCard data={card} />)
  ...
  it('投稿与采集角标', () => {
    const { rerender } = render(<RepoCard data={card} />)
    ...
    <div className="repo-card">
      <Link className="repo-card-cover" to={`/repo/${data.id}`} aria-label={data.title}>
        ...
        <span className="hover-mask">
          <span className="mask-stat">▶ {formatCount(data.views)}</span>
          <span className="mask-stat">♥ {formatCount(data.likes)}</span>
        </span>
      </Link>
...
.hover-mask {
  ...
  gap: 16px;
  padding: 10px 12px;
  background: linear-gradient(transparent, rgba(43, 32, 16, 0.55));
  color: var(--bg1);
  font-size: 13px;
  opacity: 0;
...
// jsdom 无 IntersectionObserver；提供可控 mock：triggerEnter() 模拟哨兵进视口
...
describe('HomePage', () => {
  ...
  it('渲染卡片流', async () => {
    ...
    expect(screen.getByText('tinyfetch')).toBeInTheDocument()
  })
  ...
  it('哨兵进视口触发翻页', async () => {
    ...
    io.triggerEnter()
    await screen.findByText('wasm-notes')
    expect(useFeedStore.getState().cards).toHaveLength(12)
  })
...
describe('SearchPage', () => {
  ...
  it('命中词在标题中高亮', async () => {
    renderAt('/search?q=agent')
    const mark = (await screen.findByText('mini-agent')).querySelector('mark')
